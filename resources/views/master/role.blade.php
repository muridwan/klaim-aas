                  ...
                  <table class="table table-sm table-hover" id="data-table">
                    <thead class="text-center bg-gradient-success">
                      <tr>
                        ...
                        <th>ROLE</th>
                        <th>KETERANGAN</th>
                        <th style="width: 125px">&Sigma; PENGGUNA</th>
                      </tr>
                    </thead>
                    <tbody>
                      <?php $sum = 0; ?>
                      ...
                      <tr>
                        ...
                        <td>
                          <a class="font-weight-bold" href="{{ route('users', ['role' => $role->code]) }}">
                            {{$role->code }}
                          </a>
                        </td>
                        <td>{{ $role->name }}</td>
                        <td>{{ $role->description ?? '-' }}</td>
                        <td class="text-center">{{ $role->user_roles->count() }}</td>
                      </tr>
                      <?php $sum+=$role->user_roles->count();?>
                      @empty
